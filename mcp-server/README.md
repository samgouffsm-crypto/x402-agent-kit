...
An [MCP](https://modelcontextprotocol.io) server that exposes **Jebadiah's nine
x402 pay-per-call data APIs** as 22 MCP tools. Agent operators point their MCP
...
| InvoiceIQ — AP invoice extraction | `invoiceiq_extract` | $0.05/doc |
| TaxRate US — state sales-tax rates + nexus | `taxrate_rate`, `taxrate_nexus` | $0.03 / $0.01 |
| EntityVerify — business verification | `entityverify_entity` | $0.07/check |
...
**Run from source** (no npm publish yet — this is the supported path today):
...
Then configure your MCP client (Claude Code, etc.) to launch it over stdio:
...
```json
{
  "mcpServers": {
    "x402-services": {
      "command": "node",
      "args": ["/path/to/x402-agent-kit/mcp-server/dist/index.js"],
      "env": {
...
Fund **your own** Base wallet with a little USDC (each call costs $0.01–$0.10).
...
(An npm release as `@jebadiah/x402-services` is planned; until then, source
...
(scheme `exact` — 21 of 22 tools), base64-encodes the payment envelope, and
...
- 402 requirement **generation** is verified against all nine live services
...
`../openai-functions.json` (extracted from the live OpenAPI specs) — edit the
specs, re-run `npm run gen`.
